...
{
public:
    ServerNode(int id, int _parentId, int _parentPort, int _registerPort) : AbstractNode(id), parentId(_parentId), parentPort(_parentPort), parentRegPort(_registerPort),
    ...
    void run()
    {
        ...
        while (true)
        {
            ...
            case MessageTypes::HEARTBIT_REQUEST:
                ...
                {
                    ping = true;
                    th = std::thread([this, &time, &quit]()
                                     { bitGenerator(*this, time, quit); });
                    th.detach();
                }
